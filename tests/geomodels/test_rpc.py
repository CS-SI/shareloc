...
def test_rpc_drivers():
    ...
    # Test DIMAPv3 RPC
    id_scene = "PNEO3_202111071029101_PAN_SEN_PWOI_000005973_3_1_F_1"
    file_dimap_v3 = os.path.join(data_folder, f"rpc/RPC_{id_scene}.XML")
    fctrat_dimap_v3 = RPC.from_any(file_dimap_v3, topleftconvention=True)
    assert fctrat_dimap_v3.driver_type == "dimap_v3.0"
...
@pytest.mark.skip(reason="gdal coords are shifted by one pixel")
@pytest.mark.parametrize(
    "id_scene,lon,lat,alt, col_vt,row_vt",
    [
...
def test_rpc_direct_inverse_pneo(id_scene, col, row):
    ...
    assert row == pytest.approx(loc_g2i[0], abs=5e-2)
    assert col == pytest.approx(loc_g2i[1], abs=5e-2)
